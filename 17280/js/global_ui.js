...
function metmessagesubmit(info3,info4,info5,info6){
	...
	// 电话
	var tempph = document.myform.tel;
	var tel = /^(\(\d{3,4}\)|\d{3,4}-)?\d{7,8}(-\d{1,4})?$/;  
	var mobile = /^1[3|4|5|8][0-9]\d{4,8}$/; 
	if(tempph.value.length==0 || (!tel.test(tempph.value) && !mobile.test(tempph.value))){
		alert(info6);
	...
	//对电子邮件的验证
	/*var temp = document.myform.email;
