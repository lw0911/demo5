		...
		if(_bnrCount !=  0){
			...
			if(_bnrCount ==  0){
				 ...
				 if(str.indexOf("_nouse")==-1){
					...
					str = str.replace(".png","_nouse.png");
			 ...
			 if(str.indexOf("_nouse")!=-1){
				str = str.replace("_nouse.png",".png");
...
*/
	...
	var bannerTime=setInterval(nextPage,8000)//定时器
	$("#maincontent").mouseover(function(){//当数遍划过时改变图片和按钮背景颜色
		clearInterval(bannerTime)//停止定时器（切换）
	})
	$("#maincontent").mouseout(function(){//当鼠标离开后重新调用定时器
		bannerTime=setInterval(nextPage,8000)
	})
...
(function($){
  $.fn.VogueRollOver = function() {
    ...
    for(var i = 0 ; i < this.length ; i++){
	  ...
	  _str = _str.replace("_nouse.png",".png");
	  _str = _str.replace("_nouse.jpg",".jpg");
      _str= _str.replace(".png","_on.png");
